...
/*   Updated: 2024/12/22 00:37:17 by yuyu             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	cal_hit_parameter(t_mlx *mlx)
{
	if (mlx->hit.hit_dir == -2)
	{
		...
		mlx->hit.x_pos = mlx->info.person.y_pos + mlx->ray.dy
			* (mlx->ray.x_dist - mlx->ray.delta_x) - mlx->ray.y_pos;
	}
	else if (mlx->hit.hit_dir == -3)
	{
		mlx->hit.distance = mlx->ray.y_dist - mlx->ray.delta_y;
		mlx->hit.x_pos = mlx->info.person.x_pos + mlx->ray.dx
			* (mlx->ray.y_dist - mlx->ray.delta_y) - mlx->ray.x_pos;
	}
	else
